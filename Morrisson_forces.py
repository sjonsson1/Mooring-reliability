...
def F_morison_wind(significant_wave_height_swell, U_current_veloctiy, significant_wave_height_windsea):
    # General properties (assumes cylindrical geometry; SI units)
    rhoW = 1025
    D = np.sqrt(38/np.pi)*2
    depth = 30
    ...
    # Drag force (with drag coefficient)
    CD = 0.13
    F_drag_swell = 0.5*rhoW*CD*D*(u_swell+np.absolute(U_current_veloctiy))**2
